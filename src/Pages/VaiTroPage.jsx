...
const model1Url = getSafeBasePathUrl('/dang.glb')
const model2Url = getSafeBasePathUrl('/vladimir_lenin_portrait_monument.glb')
...
function ScrollDrivenModels() {
  ...
  useFrame((_, dt) => {
    ...
    if (pageRef.current === 0) {
      ...
      const targetPosA = new THREE.Vector3(3.5, 0.5, 2)
      ...
      // Flag on right 
      const targetPosB = new THREE.Vector3(1.5, 0, 0)
      groupB.current.position.lerp(targetPosB, 0.05)
      groupB.current.rotation.y = THREE.MathUtils.damp(groupB.current.rotation.y, 1.5, 3, dt)
  ...
  return (
    <>
      ...
      {/* Flag */}
        <group ref={groupB}>
        {/* local warm key light to make yellow pop */}
        <pointLight color={0xfff0a0} intensity={1.2} distance={6} position={[0.5, 1.2, 1]} />
        {/* slightly cool fill to keep contrast */}
        <ambientLight intensity={0.8} />
        {/* Pass tint màu vàng (ví dụ rgba(255, 251, 0, 1)) */}
        <GLBModel url={model1Url} position={[0, 0, 0]} rotationY={0} scale={0.04} opacity={1} tint={0xfbbf24} />
      </group>
    </>
  )
}
...
<div className="Scene fixed h-screen w-full top-0 pointer-events-none bg-gradient-to-br from-red-700 via-red-800 to-red-200">        <HelmetProvider>
        ...
        >
          ...
          {/* small rim/back light to lift models from background */}
      ...
      <div className="relative z-1 pointer-events-auto">
        <VaiTroCustom />
        <ScrollManager pages={[{}, {}, {}]} pathname={'/vai-tro'} />
      </div>
    </>
  )
}

export default Transition(VaiTroPage)
